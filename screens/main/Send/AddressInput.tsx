...
export const AddressInput: React.FC<AddressInputProps> = ({
  ...
  const validateAddress = () => {
    ...
    try {
      ...
      const matchedChain = testnetPrefixes.find(
        (chain) => chain.testnet === decoded.prefix,
      );
  ...
  return (
    <div
      className={cn(
        `flex items-baseline ${addBottomMargin ? 'mb-4' : ''} space-x-2`,
      )}
    >
      <label
        className={cn(`text-sm text-neutral-1 whitespace-nowrap ${labelWidth}`)}
      >
        Send to:
      </label>
