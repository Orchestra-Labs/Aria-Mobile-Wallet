...
export async function fetchWalletAssets(
  ...
  try {
    console.log(
      `Fetching assets for wallet address: ${walletAddress}, network ID: ${networkID}`,
    );
    ...
    const networkName =
      LOCAL_CHAIN_REGISTRY[networkID]?.chainName || 'Unknown Network';

    // Filter assets if coinDenoms is not empty, otherwise include all
    const filteredCoins =
      coinDenoms.length > 0
        ? coins.filter((coin) => coinDenoms.includes(coin.denom))
        : coins;
    console.log(
      'Filtered coins after applying subscription filter:',
      filteredCoins,
    );
    ...
    const walletAssets = await Promise.all(
      filteredCoins.map(async (coin: Asset) => {
        ...
        if (coin.denom.startsWith(IBC_PREFIX)) {
          ...
          const resolvedAmount = adjustAmountByExponent(
            coin.amount,
            resolvedExponent,
          );
