...
export const claimAndRestake = async (
  ...
  const delegationsArray = Array.isArray(delegations)
    ? delegations
    : [delegations];
  const delegatorAddress = delegationsArray[0].delegation.delegator_address;
  const validatorAddresses = delegationsArray.map(
...
export const stakeToValidator = async (
  ...
  const formattedAmount = (
    ...
    Math.pow(
      10,
      LOCAL_ASSET_REGISTRY[denom].exponent || GREATER_EXPONENT_DEFAULT,
    )
  ).toFixed(0);
