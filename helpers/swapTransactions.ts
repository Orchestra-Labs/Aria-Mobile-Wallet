...
export const isValidSwap = ({
  ...
  const result =
    !sendAsset.isIbc &&
    !receiveAsset.isIbc &&
    sendAsset.denom !== receiveAsset.denom;
...
const queryWithRetry = async ({
  ...
  const providers = await selectNodeProviders();
  ...
  while (numberAttempts < MAX_NODES_PER_QUERY) {
    for (const provider of providers) {
      try {
        ...
        const offlineSigner = await createOfflineSignerFromMnemonic(
  ...
  throw new Error(
    `All node query attempts failed after ${MAX_NODES_PER_QUERY} attempts.`,
  );
};
