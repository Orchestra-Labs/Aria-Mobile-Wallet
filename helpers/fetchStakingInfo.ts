...
export const fetchUnbondingDelegations = async (
  ...
  try {
    ...
    if (error.response && error.response.status === 501) {
      console.error(
        'Node query failed: Unbonding delegation endpoint returned a 501 error.',
      );
    } else {
      console.error(
...
export const fetchStakingParams = async (): Promise<StakingParams | null> => {
  try {
    ...
    if (response && 'params' in response) {
      ...
      const unbondingTimeInSeconds = parseInt(
        stakingParams.unbonding_time || '0',
        10,
      );
  ...
  try {
    ...
    const combinedData: CombinedStakingInfo[] = validators.map((validator) => {
      ...
      const rewardInfo = rewards.find(
        (reward) => reward.validator === validator.operator_address,
      );
