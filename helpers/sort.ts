...
export function filterAndSortAssets(
  ...
  return finalAssets.sort((a, b) => {
    ...
    if (sortType === 'name') {
      valueA = stripNonAlphanumerics(
        a.symbol?.toLowerCase() || a.denom.toLowerCase(),
      );
      valueB = stripNonAlphanumerics(
        b.symbol?.toLowerCase() || b.denom.toLowerCase(),
      );
...
const hasUserActivity = (validator: CombinedStakingInfo) => {
  ...
  const isUnbondingFrom =
    validator.unbondingBalance &&
    parseFloat(validator.unbondingBalance.balance) > 0;
  ...
  return finalValidators.sort((a, b) => {
    ...
    switch (sortType) {
      case ValidatorSortType.NAME:
        valueA = stripNonAlphanumerics(
          a.validator.description.moniker.toLowerCase(),
        );
        valueB = stripNonAlphanumerics(
          b.validator.description.moniker.toLowerCase(),
        );
        return sortOrder === 'Asc'
          ? valueA.localeCompare(valueB, undefined, { sensitivity: 'base' })
          : valueB.localeCompare(valueA, undefined, { sensitivity: 'base' });
      ...
      case ValidatorSortType.REWARDS:
        valueA = a.rewards.reduce(
          (sum, reward) => sum + parseFloat(reward.amount),
          0,
        );
        valueB = b.rewards.reduce(
          (sum, reward) => sum + parseFloat(reward.amount),
          0,
        );
        break;
