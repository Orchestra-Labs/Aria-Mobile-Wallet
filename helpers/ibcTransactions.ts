...
const getValidIBCChannel = async (
  ...
  try {
    ...
    if (decodedSenderAddress.prefix === decodedRecipientAddress.prefix)
      return null;
    ...
    if (!matchedSendChain) {
      console.error(
        `Prefix ${decodedRecipientAddress.prefix} does not match any known chain.`,
      );
      return null;
    }
  ...
  try {
    ...
    return (
      response.channels?.filter(
        (channel: IBCChannel) => channel.state === 'STATE_OPEN',
      ) || []
    );
  } catch (error) {
    console.error(
      'Error fetching active IBC channels:',
      error,
      ' details: ',
...
export const fetchIbcPaths = async (network: NetworkLevel): Promise<any[]> => {
  ...
  const storedData = await getIBCConnections(network);
...
export const fetchIBCFile = async (
  ...
  const storedFile = await getIBCFile(network, file.name);
