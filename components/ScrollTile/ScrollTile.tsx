...
export const ScrollTile = ({
  ...
  const subtitleColor = selectTextColorByStatus(
    subtitleStatus,
    'text-neutral-1',
  );
  const secondarySubtitleColor = selectTextColorByStatus(
    secondarySubtitleStatus,
    'text-neutral-1',
  );

  const baseClasses =
    'p-2 min-h-[52px] rounded-md flex items-center cursor-pointer border';
  ...
  return (
    <div className={tileClasses} onClick={onClick}>
      ...
      <div className="flex flex-col ml-3 select-none">
        <h6
          className={`text-base ${textColor} text-left line-clamp-1 select-none`}
        >
          {formattedTitle}
        </h6>
        <p
          className={`text-xs ${subtitleColor} text-left line-clamp-1 select-none`}
        >
          {subtitle}
        </p>
      </div>
      ...
      {!secondarySubtitle && (
        <div className="text-white text-h6 line-clamp-1 select-none">
          {value}
        </div>
      )}
      {secondarySubtitle && (
        <div className="flex flex-col ml-3 select-none">
          <h6
            className={`text-base ${textColor} text-right line-clamp-1 select-none`}
          >
            {value}
          </h6>
          <div
            className={`text-xs ${secondarySubtitleColor} text-right line-clamp-1 select-none`}
          >
            {secondarySubtitle}
          </div>
        </div>
