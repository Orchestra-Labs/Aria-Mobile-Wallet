...
export const ValidatorSelectDialog: React.FC<ValidatorSelectDialogProps> = ({
  ...
  const [selectedValidators, setSelectedValidators] = useAtom(
    selectedValidatorsAtom,
  );
  ...
  const allValidatorsSelected =
    selectedValidators.length === filteredValidators.length;
  ...
  const updateFee = async () => {
    ...
    try {
      let totalAmount = isClaimDialog
        ? calculateTotalRewards()
        : calculateTotalDelegations();
      ...
      if (simulationResult) {
        formatFee(
          totalAmount,
          parseFloat(simulationResult.data?.gasWanted || '0'),
        );
      }
    } catch (error) {
      ...
      <div className="flex flex-col h-full">
        ...
        <div className="flex justify-between items-center text-blue text-sm font-bold w-full">
          ...
          <p className={simulatedFee?.textClass}>
            {simulatedFee && selectedValidators.length > 0
              ? simulatedFee.fee
              : '-'}
          </p>
        </div>
