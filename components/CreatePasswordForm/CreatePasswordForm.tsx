...
export const CreatePasswordForm: React.FC<CreatePasswordFormProps> = ({
  ...
  const [confirmPasswordStatus, setConfirmPasswordStatus] =
    useState<InputStatus>(InputStatus.NEUTRAL);
  ...
  const validateConfirmPassword = () => {
    ...
    setConfirmPasswordStatus(
      isConfirmPasswordValid ? InputStatus.SUCCESS : InputStatus.ERROR,
    );
  };
  ...
  const handleConfirmPasswordPaste = (
    ...
    <>
      <form className={includeFormClasses ? 'mt-9 flex-1' : ''}>
        ...
        <Input
          ...
          icon={
            passwordVisible ? <EyeOpen width={20} /> : <EyeClose width={20} />
          }
          iconRole="button"
        ...
        <Input
          ...
          messageText={
            confirmPasswordStatus === 'error' ? 'Passwords do not match' : ''
          }
