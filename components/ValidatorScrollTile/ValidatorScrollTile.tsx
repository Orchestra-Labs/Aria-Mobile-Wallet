...
export const ValidatorScrollTile = ({
  ...
  const { validator, delegation, balance, rewards, unbondingBalance } =
    combinedStakingInfo;
  ...
  const userHasUnbonding =
    unbondingBalance && parseFloat(unbondingBalance?.balance || '') > 0;
  const formattedRewardAmount = formatBalanceDisplay(
    strippedRewardAmount,
    symbol,
  );
  ...
  const isSelected = selectedValidators.some(
    (v) =>
      ...
      combinedStakingInfo.delegation.validator_address,
  );
  ...
  if (validator.jailed) {
    ...
    const formattedDelegatedAmount = formatBalanceDisplay(
      `${delegatedAmount}`,
      symbol,
    );
  ...
  const handleTransactionSuccess = (
    ...
    const slideTrayIsOpen =
      slideTrayRef.current && slideTrayRef.current.isOpen();
    ...
    if (slideTrayIsOpen) {
      ...
      const displayTransactionHash = truncateWalletAddress(
        '',
        txHash as string,
      );
  ...
  const formatFee = (gasWanted: number) => {
    ...
    if (selectedAction === 'claim') {
      feePercentage = feeInGreaterUnit
        ? (feeInGreaterUnit / rewardAmount) * 100
        : 0;
            ...
            {rewards && (
              <div className="text-center mb-2">
                <div className="truncate text-base font-medium text-neutral-1">
                  Reward:{' '}
                  <span className="text-blue">{formattedRewardAmount}</span>
                </div>
                <span className="text-grey-dark text-xs text-base">
                  Unstaking period{' '}
                  <span className="text-warning">{unbondingDays}</span>
                </span>
              </div>
            )}
              ...
              {transactionSuccess.success && (
                <div className="flex-grow">
                  <TransactionResultsTile
                    isSuccess
                    size="sm"
                    txHash={truncateWalletAddress(
                      '',
                      transactionSuccess.txHash as string,
                    )}
                  />
                </div>
              )}
